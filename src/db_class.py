...
class Db:
    ...
    # --- 共通部分 ---
    def remove_duplicate_rows_keep_one_dir(self, db_dir):
    ...
    def create_definition_db(self, db_path):
        conn = sqlite3.connect(db_path)
        cursor = conn.cursor()
        cursor.execute("CREATE TABLE IF NOT EXISTS my_table (def_file TEXT, class_name TEXT, func_name TEXT, id INTEGER PRIMARY KEY AUTOINCREMENT)")
        conn.commit()
        conn.close()
    ...
    def insert_link_db(self, db_path, caller_file, key_id):
        conn = sqlite3.connect(db_path)
        cursor = conn.cursor()
        cursor.execute("INSERT INTO my_table (caller_file, id) VALUES(?, ?)", (caller_file, key_id))
        conn.commit()
        conn.close()
    
    def get_id_from_def_db(self, def_db_path, func_name, class_name):
        conn = sqlite3.connect(def_db_path)
        cursor = conn.cursor()
        if class_name:
            cursor.execute("SELECT id FROM my_table WHERE func_name = ? AND class_name = ?", (class_name, func_name))
        else:
            cursor.execute("SELECT id FROM my_table WHERE func_name = ?", (func_name,))
                ...
                if key == 'type' and value in ['import_statement', 'import_from_statement']:
                    if value == 'import_from_statement':
                        ...
                        if aliased_import:
                            ...
                            self.insert_import_db(db_path, dotted_names[0], imported, alias)
                        elif len(dotted_names) > 1:
                            self.insert_import_db(db_path, dotted_names[0], dotted_names[1], None)
                        ...
                        if 'aliased_import' in [child['type'] for child in data['children']]:
                            ...
                            if imported:
                                self.insert_import_db(db_path, None, imported[0], alias)
                        else:
                            imported = [child['content'] for child in data['children'] if child['type'] == 'dotted_name']
                            if imported:
                                self.insert_import_db(db_path, None, imported[0], None)
                else:
            ...
            for child in node["children"]:
                if child["type"] == "attribute":
                    ...
                    if function_name and alias_name:
                        id = self.get_id_from_def_db(def_db_path, function_name, None)
                        self.insert_link_db(db_path, file_name, id)
        for child in node.get("children", []):
            self.extract_function_calls(child, file_name, db_path, def_db_path)
    
    # --- db main ---
    def import_attribute_db_main(self, directory, import_dir, attribute_dir):
        import_db_elements = ["from_file", "import_file", "alias"]
        attribute_db_elements = ["caller_file", "from_file", "import_file", "alias",  "func_name"]
        for filename in os.listdir(directory):
            if filename.endswith(".yaml"):
                import_name = f"import_{os.path.splitext(filename)[0]}.db"
                import_path = os.path.join(import_dir, import_name)
                attribute_name = f"attribute_{os.path.splitext(filename)[0]}.db"
                attribute_path = os.path.join(attribute_dir, attribute_name)
                self.create_db_import_elements(import_path, import_db_elements)
                self.create_db_import_elements(attribute_path, attribute_db_elements)
                with open(os.path.join(directory, filename)) as f:
                    data = yaml.safe_load(f)
                    self.extract_import_statements(data, filename, import_path)
                    self.insert_call_attribute_db(import_path, attribute_path)
                    print(f"complete : {os.path.splitext(filename)[0]}")
        self.remove_duplicate_rows_keep_one_dir(import_dir)
        self.remove_duplicate_rows_keep_one_dir(attribute_dir)
        
    def definition_db_main(self, directory, db_path):
        self.create_definition_db(db_path)
        for filename in os.listdir(directory):
            if filename.endswith(".yaml"):
                with open(os.path.join(directory, filename)) as f:
                    data = yaml.safe_load(f)
                    ...
                    for definition in definitions:
                        ...
                        id = definition["id"]
                        self.insert_definition_db(db_path, def_file, class_name, func_name)
        self.remove_duplicate_rows_keep_one(db_path)
    
    def link_db_main(self, directory, db_path, def_db_path):
        self.create_link_db(db_path)
        for filename in os.listdir(directory):
            if filename.endswith(".yaml"):
                with open(os.path.join(directory, filename)) as f:
                    data = yaml.safe_load(f)
                    self.extract_function_calls(data, filename, db_path, def_db_path)
        self.remove_duplicate_rows_keep_one(db_path)
    ...
    def get_function_ast(self, ast_dir_path, db_path, function_id):
        ...
        if result is not None:
            ...
            ast_node = analyzer.load_ast_from_file(ast_dir_path, def_file)
            if ast_node is not None:
                return analyzer.find_function_ast(ast_node, func_name)
        return None
